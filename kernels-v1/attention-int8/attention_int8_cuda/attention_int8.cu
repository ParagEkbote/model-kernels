...
load_and_quantize_K_transposed(const float16 *__restrict__ K_src,
                               int8_t *__restrict__ K_i8_T, int k_size, int tid,
                               float inv_scale_K) {
...
__device__ __forceinline__ void pad_Q_rows(int8_t *Q_i8, int q_size, int tid) {
    ...
    int  B, int H, int kv_H, int N)
{
  ...
  using FragB =
      wmma::fragment<wmma::matrix_b, 16, 16, 16, int8_t, wmma::col_major>;
  ...
  for (int k_start = 0; k_start < N; k_start += BK) {
    ...
    // WMMA path: sm_75+ (Turing, Ampere, Ada, Hopper...)
    {
      ...
      for (int wt = wid; wt < total_tiles; wt += WARPS) {
        ...
        for (int d16 = 0; d16 < nD16; ++d16) {
          ...
          wmma::load_matrix_sync(fa, Q_i8 + qi16 * 16 * HEAD_DIM + d16 * 16,
                                 HEAD_DIM);

          wmma::load_matrix_sync(fb, K_i8_T + d16 * 16 * BK + ki16 * 16, BK);

          wmma::mma_sync(acc, fa, fb, acc);
        }

        wmma::store_matrix_sync(QK_i32 + qi16 * 16 * BK + ki16 * 16, acc, BK,
                                wmma::mem_row_major);
      }
    }
#else
    ...
    for (int qi = wid; qi < BQ; qi += WARPS) {
      for (int ki = 0; ki < BK; ++ki) {
        ...
        for (int d = 0; d < HEAD_DIM; ++d)
          sum +=
              (int32_t)Q_i8[qi * HEAD_DIM + d] * (int32_t)K_i8_T[d * BK + ki];
        QK_i32[qi * BK + ki] = sum;
      }
    }
#endif
    ...
    for (int qi = wid; qi < q_size; qi += WARPS) {
      ...
      for (int ki = 0; ki < k_size; ++ki) {
        ...
        tsum += w;
        for (int d = 0; d < HEAD_DIM; ++d)
          out_acc[qi * HEAD_DIM + d] +=
              w * __half2float(V_tile[ki * HEAD_DIM + d]);
      }
      row_sum[qi] += tsum;
      row_max[qi] = new_max;
    }
...
inline size_t int8_attention_smem_bytes(int D) {
  int BQ = (D <= 128) ? 64 : 32;
  int BK = (D <= 128) ? 64 : 32;
  return (size_t)BQ * D * sizeof(int8_t) + (size_t)D * BK * sizeof(int8_t) +
         (size_t)BK * D * sizeof(float16) + (size_t)BQ * BK * sizeof(int32_t) +
         (size_t)WARPS * sizeof(float) + (size_t)BQ * 2 * sizeof(float) +
...
// Launcher [G4][G7]
// ============================================================================
cudaError_t launch_int8_attention(const float16 *Q, const float16 *K,
                                  const float16 *V, float16 *O,
...
#define LAUNCH_KERNEL(HD, CAU)                                                 \
  ...
  if (causal) {
    DISPATCH_HD(D, true, LAUNCH_KERNEL(HD, true));
  } else {
    DISPATCH_HD(D, false, LAUNCH_KERNEL(HD, false));
  }
...
void int8_attention_check_inputs(torch::Tensor Q, torch::Tensor K,
  ...
  TORCH_CHECK(D == 32 || D == 64 || D == 80 || D == 96 || D == 128 ||
                  D == 160 || D == 256,
              "HEAD_DIM must be one of: 32, 64, 80, 96, 128, 160, 256");

  // kv_H validity [G7]
  TORCH_CHECK(kv_H > 0 && kv_H <= H, "kv_H must be > 0 and <= H");
...
torch::Tensor int8_attention(torch::Tensor Q, torch::Tensor K, torch::Tensor V,
                             torch::Tensor timestep_scales, int64_t timestep,
                             bool causal) {
  ...
  cudaError_t err = launch_int8_attention(
      ...
      reinterpret_cast<float16 *>(O_fp16.data_ptr<at::Half>()), ts_ptr,
      static_cast<int>(timestep), B, H, kv_H, N, D, causal, stream);

  TORCH_CHECK(err == cudaSuccess,
              "int8_attention kernel launch failed: ", cudaGetErrorString(err));
